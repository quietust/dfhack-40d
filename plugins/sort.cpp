...
template<class T>
void reorder_vector(stl::vector<T> *pvec, const std::vector<unsigned> &order)
{
...
template<class T>
bool compute_order(color_ostream &out, lua_State *L, int base, std::vector<unsigned> *order, const stl::vector<T> &key)
{
...
/*
 * Sort units in the 'unit' and 'j'ob list screens.
 */

DEFINE_SORT_HANDLER(unit_sorters, unitjobs, "", jobs)
{
